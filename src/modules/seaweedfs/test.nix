        ...
        toh.test.commands.perNode = [
          ...
          (node: ''
            command_node.wait_until_succeeds("""
              curl -f http://${node.toh.meta.network.ip}:9333/cluster/status
            """, timeout=60)
          '')
          (node: ''
            command_node.wait_until_succeeds("""
              curl -f http://${node.toh.meta.network.ip}:8888/
            """, timeout=60)
          '')
          (node: ''
            command_node.succeed("""
              curl -f http://${node.toh.meta.network.ip}:8081/status | \
                grep -q 'Version'
            """)
          '')
          (
            ...
            builtins.map (
              ...
              if other.toh.meta.network.ip == node.toh.meta.network.ip then
                ""
              else
                ''
                  command_node.succeed("""
                    curl -f http://${node.toh.meta.network.ip}:9333/cluster/status | \
                      grep -q '${other.toh.meta.network.ip}'
                  """)
                ''
            ) nodea
          )
