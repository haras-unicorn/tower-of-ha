...
{
  flake.nixosModules.services-seaweedfs =
    {
      lib,
      config,
      pkgs,
    ...
    let
      ...
      ) (builtins.filter (host: host.name != config.toh.host.name) hosts);
    ...
    in
    {
      ...
      config = lib.mkIf config.toh.seaweedfs.enable {
        services.seaweedfs.master.enable = true;
        services.seaweedfs.master.openFirewall = true;
        services.seaweedfs.master.ip = config.toh.host.ip;
        ...
        services.seaweedfs.volumes.toh.ip = config.toh.host.ip;
        ...
        services.seaweedfs.filers.toh.ip = config.toh.host.ip;
        ...
        services.seaweedfs.filers.toh.config.postgres = {
          enabled = true;
          hostname = config.toh.database.host;
          port = config.toh.database.port;
          username = config.toh.database.instances.seaweedfs.user;
          database = config.toh.database.instances.seaweedfs.name;
        };
          ...
          secrets.generations = [
            {
              ...
              arguments = {
                ...
                # TODO: remove mention of postgres here
                text = config.toh.database.instances.seaweedfs.passwordSecret;
              };
            }
            {
              generator = "env";
