...
{
  flake.nixosModules.services-miniflux =
    {
      lib,
      config,
      pkgs,
    ...
    let
      ...
      minifluxUser = "miniflux_${config.toh.host.name}";
    ...
    in
    {
      ...
      config = lib.mkIf config.toh.miniflux.enable {
        ...
        services.miniflux.config = {
          ...
          RUN_MIGRATIONS = 1;
          CREATE_ADMIN = 1;
          BASE_URL = "https://miniflux.${config.toh.domains.service}/";
        };
        ...
        toh.cryl.host.miniflux = {
          imports = [
            {
              importer = "copy";
              arguments = {
                from = "${self.lib.cryl.directories.cluster}/${user}-password";
                to = "${user}-password";
                allow_fail = false;
              };
            }
