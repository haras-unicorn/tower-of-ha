...

{
  ...
  perSystem =
    { pkgs, ... }:
    {
      checks.test-services-consul-disabled = pkgs.tohPackages.testers.runToHTest {
        name = "services-consul-disabled";
        ...
        toh.test.disabledService.name = "consul.service";
        ...
        toh.test.commands.suffix =
          ...
          ''
            ...
            machine.wait_until_succeeds("""
              test -n "$(curl -sk https://${nodes.machine.toh.host.ip}:8500/v1/status/leader)"
            """)
            machine.wait_until_succeeds("""
              curl -sk https://${nodes.machine.toh.host.ip}:8500/v1/status/leader | \
                grep -Eq '[0-9]+.[0-9]+.[0-9]+.[0-9]+:[0-9]+'
            """)
            machine.wait_until_succeeds("""
              curl -sk https://${nodes.machine.toh.host.ip}:8500/v1/agent/self | \
                grep -q 'machine'
            """)
            machine.wait_until_succeeds("""
              curl -sk https://${nodes.machine.toh.host.ip}:8500/v1/catalog/datacenters | \
                grep -q 'toh'
            """)
            ...
            machine.fail("""
              grep -r 'disable-dnssec-toh' /etc/NetworkManager/dispatcher.d/ 2>/dev/null
            """)
          '';
      };
      ...
      checks.test-services-consul-services = pkgs.tohPackages.testers.runToHTest {
        ...
        toh.test.commands.suffix =
          ...
          ''
            ...
            """)
            machine.wait_until_succeeds("""
              curl -sk https://${nodes.machine.toh.host.ip}:8500/v1/catalog/service/test-service | \
                grep -q 'test-service'
            """)
            machine.wait_until_succeeds("""
              curl -sk https://${nodes.machine.toh.host.ip}:8500/v1/catalog/service/test-service | \
            ...
            """)
            machine.wait_until_succeeds("""
              curl -sk https://${nodes.machine.toh.host.ip}:8500/v1/catalog/service/consul-ui | \
                grep -q 'consul-ui'
            """)
          '';
      };
      ...
      checks.test-services-consul-cluster = pkgs.tohPackages.testers.runToHTest {
        ...
        toh.test.commands.perNode = [
          ...
          (node: ''
            command_node.succeed("grep '${node.toh.host.name}' /etc/consul.json")
          '')
          (node: ''
            command_node.succeed("grep -q '${node.toh.host.ip}' /etc/consul.json")
          '')
          (
            { nodea, ... }:
            builtins.map (other: ''
              command_node.succeed("grep -q '${other.toh.host.ip}' /etc/consul.json")
            '') nodea
          )
          ''
          ...
          (node: ''
            command_node.wait_until_succeeds("""
              curl -sk https://${node.toh.host.ip}:8500/v1/status/leader | \
                grep -Eq '[0-9]+.[0-9]+.[0-9]+.[0-9]+:[0-9]+'
            """)
          '')
          (
            ...
            builtins.map (other: ''
              command_node.wait_until_succeeds("""
                curl -sk https://${node.toh.host.ip}:8500/v1/agent/members | \
                  grep -Eq '${other.toh.host.name}'
              """)
            '') nodea
          )
        ];
        toh.test.commands.suffix = nodes: ''
          leader_output = node1.succeed("curl -sk https://${nodes.node1.toh.host.ip}:8500/v1/status/leader")
