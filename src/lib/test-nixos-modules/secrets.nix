        ...
        defaults =
          { config, ... }:
          {
            ...
            cryl.specification = builtins.zipAttrsWith (_: builtins.concatLists) (
              builtins.attrValues config.toh.cryl.host
            );

            environment.sessionVariables = {
              TOH_SECRETS_HOSTS = "${testConfig.cryl.sops.package}";
              TOH_SECRETS_CLUSTER = "${testConfig.cryl.sops.package}/cluster";
              TOH_SECRETS_EXTERNAL = "${testConfig.cryl.sops.package}/external";
            };
          };
      };
    };
